 ...
 * least one ``hidden'' layer of units between the inputs and the outputs (AIMA
 * Section 18.7.3).
 */
public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	 ...
	 * layers of Units (InputUnits in the first layer, LogisticUnits in other
	 * layers). It's up to you to arrange the feed-forward connections between the
	 * Units properly.
	 */
	 ...
	 * and the given number of output units, where each layer is fully connected to
	 * the next.
	 */
	...
	/**
	 * Construct and return a new MultiLayerFeedForwardNeuralNetwork with the given
	 * number of input units, a single hidden layer of the given length, and the
	 * given number of output units, where each layer is fully connected to the
	 * next.
	 */
	public MultiLayerFeedForwardNeuralNetwork(int ninputs, int nhiddens, int noutputs) {
	...
	/**
	 * Print this MultiLayerFeedForwardNeuralNetwork to stdout. All we print the
	 * weights of each unit in each non-input layer in tab-separated format:
	 * LAYERNUM UNITNUM w_0 w_1 ... w_n.
	 */
	public void dump() {
	 ...
	 * (i.e., class label) with the maximum activation. You could override this in a
	 * subclass if you wanted something different.
	 */
	...
	public void train(List<Example> examples, int epochs, double alpha) {
		initializeWeights();
		// ``until some stopping criterion is specified'' (we use a fixed number of
		// epochs)
	...
	/**
	 * AIMA Fig 18.24 says weights should each be initialized to ``a small random
	 * number.'' WTF? It also has that step inside the learning loop, which strikes
	 * me as very wrong. For now I'm setting all non-input unit weights to w in
	 * range [-0.05,0.05].
	 */
	...
	/**
	 * AIMA Fig 18.14 body of loop looks like three steps, but it's really four: (1)
	 * ``Propagate the inputs forward to compute the outputs'' (2) Computing the
	 * error vector delta (3) ``Propagating deltas backward from output layer to
	 * input layer'' (4) ``Update every weight in network using deltas''
	 */
	...
	/**
	 * AIMA Fig 18.14: Body of main loop, step 1: ``Propagate the inputs forward to
	 * compute the outputs.''
	 */
	...
	/**
	 * AIMA Fig 18.14 body of loop after propagating inputs forward: (2) Computing
	 * the error vector delta (3) ``Propagating deltas backward from output layer to
	 * input layer'' (4) ``Update every weight in network using deltas''
	 */
	public void backprop(Example example, double alpha) {
		...
		for (int j = 0; j < outputs.length; j++) {
			...
			LogisticUnit unit = (LogisticUnit) outputs[j];
			unit.delta = unit.activationPrime(unit.getInputSum()) * (example.outputs[j] - unit.getOutput());
		}
		...
		for (int l = this.layers.length - 1; l >= 1; l--) {
			...
			for (int i = 0; i < this.layers[l].length; i++) {
				...
				for (Connection conn : unit.incomingConnections) {
					NeuronUnit j = (NeuronUnit) conn.dst;
					unit.delta += unit.activationPrime(unit.getInputSum()) * conn.weight * j.delta;
				}
			}
		}
		...
		for (int l = 1; l < this.layers.length; l++) {
			...
			for (int i = 0; i < units.length; i++) {
				...
				for (Connection conn : unit.outgoingConnections) {
					NeuronUnit dst = (NeuronUnit) conn.dst;
					conn.weight += alpha * conn.src.getOutput() * dst.delta;
				}
			}
		}
	 ...
	 * on the given Example. ``Getting the right answer'' depends on how the problem
	 * is represented in the network. This default implementation assumes that there
	 * is one output unit per class label (that is, a vector output). In the
	 * Example, only one of these will be 1.0 and the others will be 0.0. This
	 * method tests that the index of the output unit with the highest activation
	 * (the network's ``output'') is the index of the 1.0 in the Example's outputs.
	 * Other things are possible, in which case subclasses can override this
	 * implementation.
	 */
	@Override
	public boolean test(Example example) {
		...
		// Return true if that's the one that's supposed to be on according to the
		// example
	...
	/**
	 * Run a k-fold cross-validation experiment on this
	 * MultiLayerFeedForwardNeuralNetwork using the given Examples and return the
	 * average accuracy over the k trials. For this type of NeuralNetwork, we use a
	 * closure to pass the number of epochs and learning rate alpha into the
	 * training procedure.
	 */
	public double kFoldCrossValidate(List<Example> examples, int k, int epochs, double alpha, boolean print) {
		...
		return super.kFoldCrossValidate(examples, k, trainer, tester, print);
	}
