...
/**
 * Base class for the non-input units of a NeuralNetwork. Do not include an
 * input for the constant a_0=1.0 (AIMA p. 728) since that will be added
 * automatically.
 */
abstract public class NeuronUnit extends Unit {
	...
	/**
	 * Return the weight for this NeuronUnit's i'th input (where input 0 is the
	 * constant bias).
	 */
	...
	/**
	 * Set the weight for this NeuronUnit's i'th input (where input 0 is the
	 * constant bias).
	 */
	...
	/**
	 * Return the weighted sum of this NeuronUnit's inputs. This is also used in the
	 * backpropagation algorithm which is why I broke it out from the run() method.
	 */
	...
	/**
	 * ``Each unit j first computes a weighted sum of its inputs. Then it applies an
	 * activation function g to this sum to derive the output.'' (AIMA p728).
	 */
	@Override
	public void run() {
	...
	/**
	 * The activation (threshold) function used by this NeuronUnit. This method must
	 * be specified by subclassses.
	 */
	abstract public double activation(double in);

	/**
	 * Update the weights of this NeuronUnit using the given input values, the given
	 * output value, and learning rate (alpha). This method must be specified by
	 * subclassses.
	 */
	@Override
	...
	/**
	 * Return the weighted sum of a given delta. This is also used in the
	 * backpropagation algorithm.
	 */
	public double getDeltaSums(double delta) {
		double sum = 0;
		for (Connection conn : this.outgoingConnections) {
			sum += conn.weight * delta;
		}
		return sum;
	}
